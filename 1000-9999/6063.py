...
for sc in range(t):
    ...
    for _ in range(s):
        ...
        spin=['left','front','right','back','top','bottom']
        if i[1]=='-1':
            eval(spin[int(i[0])]+'(cube)')
        else: 
            for _ in range(3):
                eval(spin[int(i[0])]+'(cube)')
    
    print("Scenario #"+str(sc+1)+":")
    print("      "+' '.join(cube[0][0:3]))
    print("      "+' '.join(cube[0][3:6]))
    print("      "+' '.join(cube[0][6:9]))
    for i in range(3):
        for j in range(12):
            print(cube[j//3+1].pop(0), end=' ')
        print()
    print("      "+' '.join(cube[5][0:3]))
    print("      "+' '.join(cube[5][3:6]))
    print("      "+' '.join(cube[5][6:9]))
    print()